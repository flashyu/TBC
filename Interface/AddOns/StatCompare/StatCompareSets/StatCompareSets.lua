...
local SCS_Stats_list = {
	...
	{name = SCS_STR,	value = "STR"},
	{name = SCS_SPI,	value = "SPI"},
	{name = SCS_HEAL,	value = "HEAL"},
	{name = SCS_DMG,	value = "DMG"},
	{name = SCS_AP,		value = "ATTACKPOWER"},
	...
	{name = SCS_RESILIENCE,	value = "RESILIENCE"},
	{name = SCS_HASTESPELL,	value = "HASTESPELL"},
	{name = SCS_HASTEMELEE,	value = "HASTEMELEE"},
	{name = SCS_IGNOREARMOR,	value = "NOARM"},
	};
...
local function StatCompare_GetAllItems()
	...
	for id = 1, MAXID do
		if not SCS_DB[id] then
			local name, _, quality, _, _, itemType, itemSubType, _, itemEquipLoc, texture = GetItemInfo(id);
			--[[
			    * 0 = Poor
			    * 1 = Common
			    * 2 = Uncommon
...
--[[
	for i = 1, 19 do
		if(StatCompare_BestItems[i] and StatCompare_BestItems[i]["HEAL"]) then
			...
			local link = hex .. "|H".. "item:" .. id .. ":0:0:0:0:0:0:0" .. "|h[" .. SCS_DB[id].name .. "]|h|r";
