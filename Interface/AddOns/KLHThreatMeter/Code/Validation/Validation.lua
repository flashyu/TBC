...
-- Note: this is the legacy validation module. It's due to be replaced by TypeSchema, TypeValidation etc.
...
me.validatearg = function(module, actualargs, argname, argdata)
	...
	if argdata.type == "enum" then
		...
		if not mod.enum.contains(argdata.enumname, actualvalue) then
			return mod.format("The value '%s' is not allowed in the '%s.%s' enumeration. Possible values are %s.", tostring(actualvalue), module.name, argdata.enumname, mod.enum.valuesettostring(argdata.enumname))
		end
...
me.processarg = function(argdata, argresult, source, istableschema)
	...
	if argdata.type then
		...
		if not mod.enum.contains("argtypes", argdata.type) then
			
			-- warn and ignore
			if mod.trace.check("warning", up, "badconfig") then
				mod.trace.printf("Bad Validation data in %s: the value of the 'type' property, '%s', does not satisfy the 'argtypes' Enumeration. Allowed values are %s.", source, argdata.type, mod.enum.valuesettostring("argtypes"))
			end
...
me.addproperty = function(propertyname, propertydata, argresult, source, istableschema)
	...
	elseif propertyname == "set" then
		...
		if not mod.enum.contains("numbersets", propertydata) then
		
			-- warn and ignore
			if mod.trace.check("warning", up, "badconfig") then
				mod.trace.printf("Bad Validation data in module %s: the 'set' property does not satisfy the 'numbersets' Enumeration. Allowed values are %s.", source, mod.enum.valuesettostring("numbersets"))
			end
	...
	elseif propertyname == "enumname" then
		...
		if not mod.enum.isdefined(propertydata) then
