...
function HealBot_HealthColor(unit,hlth,maxhlth,tooltipcol,Member_Name,UnitDead,Member_Buff,Member_Debuff,healin)
  ...
  if not tooltipcol then
    ...
    elseif Member_Buff then
        ...
        --return hcr,hcg,hcb,HealBot_Config.Barcola[HealBot_Config.Current_Skin],hrpct
  ...
  if hcpct>HealBot_Config.AlertLevel then
    ...
    --hca=HealBot_Config.Barcola[HealBot_Config.Current_Skin]
...
function HealBot_CorrectPetHealth(unit,hlth,maxhlth,Member_Name)
  if not HealBot_PetMaxH[Member_Name] then HealBot_PetMaxH[Member_Name]=hlth; end
  if hlth>HealBot_PetMaxH[Member_Name] then HealBot_PetMaxH[Member_Name]=hlth; end
  if HealBot_PetMaxH[Member_Name] and not HealBot_IsFighting then
    if not HealBot_PetMaxHcnt1[Member_Name] then 
      HealBot_PetMaxHcnt1[Member_Name]=1
      HealBot_PetMaxHcnt2[Member_Name]=HealBot_PetMaxH[Member_Name];
    else
      if HealBot_PetMaxHcnt2[Member_Name]~=hlth then
        HealBot_PetMaxHcnt2[Member_Name]=hlth;
        HealBot_PetMaxHcnt1[Member_Name]=1;
      else
        HealBot_PetMaxHcnt1[Member_Name]=HealBot_PetMaxHcnt1[Member_Name]+1;
        if HealBot_PetMaxHcnt1[Member_Name]>20 then
          HealBot_PetMaxH[Member_Name]=HealBot_PetMaxHcnt2[Member_Name];
        end
      end
    end
  end
  return HealBot_PetMaxH[Member_Name]
end

function HealBot_RetCorrectPetHealth(unit,hlth,maxhlth,Member_Name)
  if not HealBot_PetMaxH[Member_Name] then
    HealBot_CorrectPetHealth(unit,hlth,maxhlth,Member_Name)
  end
  return HealBot_PetMaxH[Member_Name]
end	
...
function HealBot_Action_EnableButton(button, ebuName)
  ...
  ebuhlth=UnitHealth(ebUnit) or 0;
  ebumaxhlth=UnitHealthMax(ebUnit) or 0;
  ...
  if not HealBot_UnitRange[ebUnit] then
    --HealBot_UnitRange[ebUnit]=UnitInRange(ebUnit) or 0
...
function HealBot_Action_HBText(hlth,maxhlth,Member_Name,unit,healin)
  ...
  if strlen(Member_Name)>bttextlen then
...
local b,bar,bar2,bar3,bar4,icon1,icon2,icon3,icon1t,icon2t,icon3t,barScale,h,hwidth,hheight
  ...
  if HealBot_Options:IsVisible() then 
    ...
    HealBot_CustomColorpick:SetStatusBarTexture("Interface\\AddOns\\HealBot\\images\\bar"..HealBot_Config.btexture[HealBot_Config.Current_Skin]);--added by Diacono
